...
var Board2 = function (size) {
    ...
    /*
     * Add a new Candy2 to the board.  Requires candies to be not currently
    ...
    */
    this.addCandy = function (color, row, col, spawnRow, spawnCol) {
        var candy = new Candy2(color, candyCounter++);
    ...
    this.addRandomCandy = function (row, col, spawnRow, spawnCol) {
        let item = window.location.pathname.split("/")[2];
        var random_color = Math.floor(Math.random() * Candy2[`colors${item}`].length);
        var candy = new Candy2(Candy2[`colors${item}`][random_color], candyCounter++);
        this.add(candy, row, col, spawnRow, spawnCol);
    };
    ...
    */
    ...
    this.incrementScore = function (candy, row, col) {
        ...
        if(candy.color === "nube" || candy.color === "flower") {
            this.score += 30;
        } else {
            this.score += 25;
        }
