window.location.pathname.includes('/world-3') &&
    setTimeout(() => {
        let e = !1;
        $.extend({
            getUrllets: function () {
                let e = [],
                    t,
                    n = window.location.search.substr(1).split('&');
                for (let i = 0; i < n.length; i++)
                    (t = n[i].split('=')),
                        e.push(t[0]),
                        (e[t[0]] =
                            t[1] &&
                            decodeURIComponent(t[1].replace(/\+/g, ' ')));
                return e;
            },
            getUrllet: function (e) {
                return $.getUrllets()[e];
            },
        });
        let t = 6,
            n,
            i,
            o = null;
        (n = new Board3(
            $.getUrllet('size') &&
            $.getUrllet('size') >= 3 &&
            20 >= $.getUrllet('size')
                ? $.getUrllet('size')
                : t
        )),
            (i = new Rules3(n)),
            $(document).ready(function () {
                if (
                    (c(),
                    localStorage.setItem('cat', JSON.stringify(8)),
                    localStorage.setItem('sun', JSON.stringify(10)),
                    localStorage.setItem('bacterium', JSON.stringify(10)),
                    localStorage.setItem('apple', JSON.stringify(10)),
                    localStorage.setItem('nube', JSON.stringify(5)),
                    localStorage.setItem('flower', JSON.stringify(5)),
                    (document.getElementById('cat-counter').innerText = 8),
                    (document.getElementById('sun-counter').innerText = 10),
                    (document.getElementById(
                        'bacterium-counter'
                    ).innerText = 10),
                    (document.getElementById('apple-counter').innerText = 10),
                    (document.getElementById('score').innerText = 0),
                    $('#clear').trigger('load'),
                    e)
                ) {
                    let t = document.createElement('div');
                    $(t).attr('id', 'DEBUG'), $('body').append(t);
                }
            }),
            $(n).on('add', function (e, t) {
                let i = t.candy,
                    o = document.createElement('img');

                    $('#desler').fadeIn('slow');

                    setTimeout(() => {
                        $('#desler').fadeOut('slow');
                    }, 200)
                $('#gameBoard').append(o),
                    'nube' === t.candy.color || 'flower' === t.candy.color
                        ? (o.src =
                              'https://juegoseml.co/images-recover/' +
                              i.toString() +
                              '-purple.gif')
                        : (o.src =
                              'https://juegoseml.co/images-recover/' +
                              i.toString() +
                              '-purple.png'),
                    $(o).data('candy', i),
                    $(o).attr('id', 'candy-id-' + i.id),
                    $(o).attr('data-position', i.col + '-' + i.row);
                let r;
                window.innerWidth > 819
                    ? (r = 650 / n.boardSize)
                    : window.innerWidth > 767
                    ? (r = 550 / n.boardSize)
                    : window.innerWidth > 400
                    ? (r = 410 / n.boardSize)
                    : (window.innerWidth, (r = 360 / n.boardSize)),
                    document.getElementById('canvas')
                        ? document.getElementById('canvas').clientHeight
                        : n.boardSize;
                let a = i.row * r,
                    l = i.col * r,
                    c = 0 - (n.boardSize - a / r) * r;
                $(o).css({
                    width: r,
                    height: r,
                    top: c,
                    left: l,
                    position: 'absolute',
                }),
                    $(o).animate({ top: a }, function () {
                        s();
                    });
            });
        let r = !1;
        function a() {
            let e = i.getRandomValidMove(),
                t = document.getElementById('canvas'),
                o = t ? t.getContext('2d') : null;
            o.clearRect(0, 0, 650, 650);
            let r = e.candy.col,
                a = e.candy.row,
                l;
            window.innerWidth > 819
                ? (l = 650 / n.boardSize)
                : window.innerWidth > 767
                ? (l = 550 / n.boardSize)
                : window.innerWidth > 400
                ? (l = 410 / n.boardSize)
                : (window.innerWidth, (l = 360 / n.boardSize));
            let s = l / 2,
                c = (r + 1) * l - s,
                f = (a + 1) * l - s;
            (o.fillStyle = '#333333'),
                o.beginPath(),
                'up' == e.direction
                    ? (o.fillRect(c - s / 2, f - s, s, s),
                      o.moveTo(c - s, f - s + 1),
                      o.lineTo(c, f - 2 * s),
                      o.lineTo(c + s, f - s + 1))
                    : 'down' == e.direction
                    ? (o.fillRect(c - s / 2, f, s, s),
                      o.moveTo(c + s, f + s - 1),
                      o.lineTo(c, f + s + s),
                      o.lineTo(c - s, f + s - 1))
                    : 'left' == e.direction
                    ? (o.fillRect(c - s, f - s / 2, s, s),
                      o.moveTo(c - s + 1, f - s),
                      o.lineTo(c - 2 * s, f),
                      o.lineTo(c - s + 1, f + s))
                    : 'right' == e.direction &&
                      (o.fillRect(c, f - s / 2, s, s),
                      o.moveTo(c + s - 1, f + s),
                      o.lineTo(c + 2 * s, f),
                      o.lineTo(c + s - 1, f - s)),
                o.closePath(),
                o.fill();
        }
        function l() {
            document
                .getElementById('canvas')
                .getContext('2d')
                .clearRect(0, 0, 650, 650);
        }
        function s() {
            setTimeout(function () {
                i.moveCandiesDown();
            }, 500),
                i.removeCrushes(i.getCandyCrushes());
        }
        function c() {
            n.clear(), n.resetScore(), i.prepareNewGame();
        }
        $(n).on('move', function (e, t) {
            let i = document.getElementById('candy-id-' + t.candy.id);
            $(i).attr('data-position', t.toCol + '-' + t.toRow);
            let o;
            window.innerWidth > 819
                ? (o = 650 / n.boardSize)
                : window.innerWidth > 767
                ? (o = 550 / n.boardSize)
                : window.innerWidth > 400
                ? (o = 410 / n.boardSize)
                : (window.innerWidth, (o = 360 / n.boardSize)),
                (r = !0);
            let a = t.toRow * o,
                l = t.toCol * o;
            $(i).animate({ top: a, left: l }, function () {
                s();
            });
        }),
            $(n).on('remove', function (e, t) {
                if ('cat' === t.candy.color) {
                    let n = parseInt(JSON.parse(localStorage.getItem('cat')));
                    n - 1 <= 0
                        ? ((document.getElementById(
                              'cat-counter'
                          ).innerText = 0),
                          localStorage.setItem('cat', JSON.stringify(0)))
                        : ((document.getElementById('cat-counter').innerText =
                              n - 1),
                          localStorage.setItem('cat', JSON.stringify(n - 1)));
                } else if ('sun' === t.candy.color) {
                    let i = parseInt(JSON.parse(localStorage.getItem('sun')));
                    i - 1 <= 0
                        ? ((document.getElementById(
                              'sun-counter'
                          ).innerText = 0),
                          localStorage.setItem('sun', JSON.stringify(0)))
                        : ((document.getElementById('sun-counter').innerText =
                              i - 1),
                          localStorage.setItem('sun', JSON.stringify(i - 1)));
                } else if ('bacterium' === t.candy.color) {
                    let o = parseInt(
                        JSON.parse(localStorage.getItem('bacterium'))
                    );
                    o - 1 <= 0
                        ? ((document.getElementById(
                              'bacterium-counter'
                          ).innerText = 0),
                          localStorage.setItem('bacterium', JSON.stringify(0)))
                        : ((document.getElementById(
                              'bacterium-counter'
                          ).innerText = o - 1),
                          localStorage.setItem(
                              'bacterium',
                              JSON.stringify(o - 1)
                          ));
                } else if ('flower' === t.candy.color) {
                    let r = parseInt(
                        JSON.parse(localStorage.getItem('flower'))
                    );
                    r - 1 <= 0
                        ? localStorage.setItem('flower', JSON.stringify(0))
                        : localStorage.setItem('flower', JSON.stringify(r - 1));
                } else if ('nube' === t.candy.color) {
                    let a = parseInt(JSON.parse(localStorage.getItem('nube')));
                    a - 1 <= 0
                        ? localStorage.setItem('nube', JSON.stringify(0))
                        : localStorage.setItem('nube', JSON.stringify(a - 1));
                } else {
                    let l = parseInt(JSON.parse(localStorage.getItem('apple')));
                    l - 1 <= 0
                        ? ((document.getElementById(
                              'apple-counter'
                          ).innerText = 0),
                          localStorage.setItem('apple', JSON.stringify(0)))
                        : ((document.getElementById('apple-counter').innerText =
                              l - 1),
                          localStorage.setItem('apple', JSON.stringify(l - 1)));
                }
                let s = parseInt(JSON.parse(localStorage.getItem('cat'))),
                    c = parseInt(JSON.parse(localStorage.getItem('sun'))),
                    f = parseInt(JSON.parse(localStorage.getItem('bacterium'))),
                    d = parseInt(JSON.parse(localStorage.getItem('apple'))),
                    g = parseInt(JSON.parse(localStorage.getItem('flower'))),
                    m = parseInt(JSON.parse(localStorage.getItem('nube')));
                0 === s &&
                    0 === c &&
                    0 === f &&
                    0 === d &&
                    0 === g &&
                    0 === m &&
                    setTimeout(() => {
                        localStorage.setItem(
                            'score',
                            JSON.stringify(this.getScore())
                        );
                        let e = new Date();
                        e.setTime(e.getTime() + 2592e6),
                            (document.cookie =
                                'score=' +
                                this.getScore() +
                                ';expires=' +
                                e.toUTCString() +
                                ';path=/');
                    }, 8e3);
                let u = document.getElementById('candy-id-' + t.candy.id);
                    $(u).animate({ opacity: 0 }, function () {
                        u.parentNode.removeChild(u);
                    });
            }),
            $(n).on('scoreUpdate', function (e, t) {
                document.getElementById('score') &&
                    (document.getElementById('score').innerText = t.score);
            }),
            $(document).on('click', '.btn', function (e) {
                let t = e.target.id;
                'newGame' == t
                    ? !1 == $('img').is(':animated') && (l(), c())
                    : 'showMove' == t &&
                      !1 == $('img').is(':animated') &&
                      (l(), a());
            }),
            $(document).on('keydown', function (e) {
                'n' == e.originalEvent.key && (l(), c());
            }),
            $(document).on('mousedown touchstart', '#canvas', function (t) {
                if (!1 == $('img').is(':animated')) {
                    let i;
                    window.innerWidth > 819
                        ? (i = 650 / n.boardSize)
                        : window.innerWidth > 767
                        ? (i = 550 / n.boardSize)
                        : window.innerWidth > 400
                        ? (i = 410 / n.boardSize)
                        : (window.innerWidth, (i = 360 / n.boardSize));
                    let r, a;
                    'mousedown' == t.type
                        ? ((r = t.offsetX), (a = t.offsetY))
                        : ((r =
                              parseInt(t.touches[0].clientX) -
                              (parseInt(t.target.offsetLeft) +
                                  parseInt(t.target.offsetParent.offsetLeft))),
                          (a =
                              parseInt(t.touches[0].clientY) -
                              (parseInt(t.target.offsetTop) +
                                  parseInt(t.target.offsetParent.offsetTop))),
                          e &&
                              (console.log(
                                  parseInt(t.changedTouches[0].clientX),
                                  '- (',
                                  parseInt(t.target.offsetLeft),
                                  '+',
                                  parseInt(t.target.offsetParent.offsetLeft),
                                  ') = ',
                                  r
                              ),
                              console.log(
                                  parseInt(t.changedTouches[0].clientY),
                                  '- (',
                                  parseInt(t.target.offsetTop),
                                  '+',
                                  parseInt(t.target.offsetParent.offsetTop),
                                  ') = ',
                                  a
                              ),
                              $('#DEBUG').empty(),
                              $('#DEBUG').append('s: ' + r + ' ' + a)));
                    let l = Math.floor(r / i),
                        s = Math.floor(a / i),
                        c = document
                            .querySelectorAll(
                                "[data-position='" + l + '-' + s + "']"
                            )
                            .item(0);
                    if (null != c) {
                        $(c).css('z-index', 2);
                        let f = parseInt($(c).css('top')),
                            d = parseInt($(c).css('left'));
                        o = {
                            candyImg: c,
                            initCol: l,
                            initRow: s,
                            initTop: f,
                            initLeft: d,
                            initXCoord: r,
                            initYCoord: a,
                        };
                    }
                }
            }),
            $(document).on('mousemove touchmove', '#canvas', function (t) {
                if (null != o && !1 == $('img').is(':animated')) {
                    let n, i;
                    'mousemove' == t.type
                        ? ((n = t.offsetX), (i = t.offsetY))
                        : (e && console.log(t),
                          (n =
                              parseInt(t.touches[0].clientX) -
                              (parseInt(t.target.offsetLeft) +
                                  parseInt(t.target.offsetParent.offsetLeft))),
                          (i =
                              parseInt(t.touches[0].clientY) -
                              (parseInt(t.target.offsetTop) +
                                  parseInt(t.target.offsetParent.offsetTop))));
                    let r = o.initTop + i - o.initYCoord,
                        a = o.initLeft + n - o.initXCoord;
                    $(o.candyImg).css({ top: r, left: a });
                }
            }),
            $(document).on('mouseup touchend', function (t) {
                if (null != o) {
                    l();
                    let r;
                    window.innerWidth > 819
                        ? (r = 650 / n.boardSize)
                        : window.innerWidth > 767
                        ? (r = 550 / n.boardSize)
                        : window.innerWidth > 400
                        ? (r = 410 / n.boardSize)
                        : (window.innerWidth, (r = 360 / n.boardSize));
                    let a, s;
                    'mouseup' == t.type
                        ? ((a = t.offsetX), (s = t.offsetY))
                        : ((a =
                              parseInt(t.changedTouches[0].clientX) -
                              (parseInt(t.target.offsetLeft) +
                                  parseInt(t.target.offsetParent.offsetLeft))),
                          (s =
                              parseInt(t.changedTouches[0].clientY) -
                              (parseInt(t.target.offsetTop) +
                                  parseInt(t.target.offsetParent.offsetTop))),
                          e &&
                              (console.log(t),
                              console.log(t.changedTouches),
                              console.log(
                                  parseInt(t.changedTouches[0].clientX),
                                  '- (',
                                  parseInt(t.target.offsetLeft),
                                  '+',
                                  parseInt(t.target.offsetParent.offsetLeft),
                                  ') = ',
                                  a
                              ),
                              console.log(
                                  parseInt(t.changedTouches[0].clientY),
                                  '- (',
                                  parseInt(t.target.offsetTop),
                                  '+',
                                  parseInt(t.target.offsetParent.offsetTop),
                                  ') = ',
                                  s
                              ),
                              $('#DEBUG').append(', e: ' + a + ' ' + s)));
                    let c = Math.floor(a / r),
                        f = Math.floor(s / r),
                        d = $(o.candyImg).data('candy');
                    o.initCol == c && o.initRow - 1 == f
                        ? i.isMoveTypeValid(d, 'up') &&
                          n.flipCandies(d, n.getCandyInDirection(d, 'up'))
                        : o.initCol == c && o.initRow + 1 == f
                        ? i.isMoveTypeValid(d, 'down') &&
                          n.flipCandies(d, n.getCandyInDirection(d, 'down'))
                        : o.initCol - 1 == c && o.initRow == f
                        ? i.isMoveTypeValid(d, 'left') &&
                          n.flipCandies(d, n.getCandyInDirection(d, 'left'))
                        : o.initCol + 1 == c &&
                          o.initRow == f &&
                          i.isMoveTypeValid(d, 'right') &&
                          n.flipCandies(d, n.getCandyInDirection(d, 'right')),
                        $(o.candyImg).css({
                            'z-index': 1,
                            top: o.initTop,
                            left: o.initLeft,
                        }),
                        (o = null);
                }
            });
    }, 2e3);
