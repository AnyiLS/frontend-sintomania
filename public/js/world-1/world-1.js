if (window.location.pathname.includes('/world-1')) {
    setTimeout(() => {
        ...
        if (
            ...
            board = new Board($.getUrllet('size'));
        } else {
        ...
        $(board).on('add', function (e, info) {
            ...
            let height = document.getElementById('canvas')
                ? document.getElementById('canvas').clientHeight
                : 0;
            let candySizeHeight = document.height / board.boardSize;
        ...
        $(board).on('remove', function (e, info) {
            ...
            if (cat === 0 && sun === 0 && bacterium === 0 && apple === 0) {
                // console.log("score", );
                setTimeout(() => {
                    localStorage.setItem(
                        'score',
